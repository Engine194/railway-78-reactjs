...
export default function UserForm({ data, pushUser, closeModal, updateUser }) {
  ...
  const handleSubmit = (event) => {
    setLoading(true);
    event.preventDefault(); // ngăn chặn sự default của form
    const userParams = {
      ...user,
          ...
          <form className="user-form" onSubmit={handleSubmit}>
            ...
            <div className="form-actions">
              <button className="form-btn btn-reset" type="reset"
                onClick={() => { setUser(data || initialUser) }}
                disabled={loading}
              >
                Reset
              </button>
